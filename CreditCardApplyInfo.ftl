...
<#assign v_alarmNo=RequestParameters["alarmNo"]?default("")>
<#assign v_alarmDate=RequestParameters["alarmDate"]?default("")>
...
var alarmNo = "${v_alarmNo}";
var alarmDate = "${v_alarmDate}";

function initCallGetter_post() {
	if(alarmNo!=""){
		CreditCardApplyInfo_interface_dataset.setValue("qalarmDateStart",alarmDate);
		CreditCardApplyInfo_interface_dataset.setValue("qalarmDateEnd",alarmDate);
		CreditCardApplyInfo_interface_dataset.setValue("qinvestigateResult","0_01");
		CreditCardApplyInfo_interface_dataset.setValue("qalarmNo",alarmNo);
		
		CreditCardApplyInfo_dataset.setParameter("qalarmDateStart",alarmDate);
		CreditCardApplyInfo_dataset.setParameter("qalarmDateEnd",alarmDate);
		CreditCardApplyInfo_dataset.setParameter("qinvestigateResult","0_01");
		CreditCardApplyInfo_dataset.setParameter("qalarmNo",alarmNo);
...
/*
function CreditCardApplyInfo_dataset_flushDataPost(dataset){
	hightlight();
}
...
function hightlight(){
	...
	while(record){
		var custCategory = record.getValue("custCategory");
		if(custCategory=="Jade"){
			$("tr[datagrid-row-index='"+indexBackGround+"']").children().css("background-color","#FFFF00");
		}
